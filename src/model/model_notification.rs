...
impl ModelNotification {
    ...
    async fn reminder_text(&self) -> String {
        ...
        let message_pool = vec![i18n_f(
            "{} steps remaining to complete your daily stepgoal: {} steps",
        ...
        let periodic = glib::clone!(@strong self as x => move || {
            glib::clone!(@weak x as obj => move || {
                glib::MainContext::default().spawn_local(
                    async move {
                        obj.imp().inner.borrow_mut().hour_count+=1;
                        let time_now = Local::now().time();
                        let notify_time = NaiveTime::parse_from_str(obj.imp().settings.notification_time().as_str(),"%H:%M:%S").unwrap();
                        let interval = match obj.imp().settings.notification_frequency() {
                            NotifyMode::Hourly => 60,
                            NotifyMode::Every4hrs => 60*4,
                            NotifyMode::Fixed => 0,
                        };
                        let fixed_time = time_now.hour() == notify_time.hour() && time_now.minute() == notify_time.minute() && obj.imp().settings.notification_frequency() == NotifyMode::Fixed;
                        let periodic = obj.imp().settings.notification_frequency() != NotifyMode::Fixed && obj.imp().inner.borrow().hour_count%interval == 0;
                        if (fixed_time || periodic) && obj.imp().settings.enable_notifications() {
                            Notification::new()
                                .summary("Health: walking reminder")
                                .body(&(obj.reminder_text().await))
